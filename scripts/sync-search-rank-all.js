...
async function main() {
  ...
  try {
    ...
    // 큰 OFFSET은 서버 타임아웃 → 7일 단위로 날짜 분할 조회 (GROUP BY에 날짜 포함이라 결과 동일)
    const rows = [];
    for (let d = 90; d > 0; d -= 7) {
      const chunk = await queryAll(`
        SELECT \`검색어\` AS kw, DATE_FORMAT(\`랭킹등록일시\`,'%Y-%m-%d') AS date, MIN(\`순위\`) AS r
        FROM db_for_ai_sm.v_analyze_search_ranking
        WHERE \`검색브랜드\` = '오아'
          AND \`랭킹등록일시\` >= DATE_SUB(CURDATE(), INTERVAL ${d} DAY)
          ${d - 7 > 0 ? `AND \`랭킹등록일시\` < DATE_SUB(CURDATE(), INTERVAL ${d - 7} DAY)` : ''}
        GROUP BY 1, 2
        ORDER BY 1, 2
      `, { quiet: true });
      rows.push(...chunk);
      process.stdout.write(`\r  조회 중... ${rows.length}행 (D-${d}~)`);
    }
    console.log('');
