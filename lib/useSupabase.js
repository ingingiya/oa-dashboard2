...
// 이미용 실판매 (channel_daily_sales) — 드라이기/고데기/갈바닉/화장거울
export async function getBeautyRealSales(fromDate) {
  const cat = "or=(category.ilike.*드라이기*,category.ilike.*고데기*,category.ilike.*갈바닉*,category.ilike.*화장거울*)";
  const res = await fetch(
    `${SUPA_URL}/rest/v1/channel_daily_sales?select=date,qty,category,channel,name&${cat}&date=gte.${fromDate}&order=date.asc`,
