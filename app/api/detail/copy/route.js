...

{
  ...
  "cutPrompts": {
    ...
    "before": "부정형(비포) 영어 프롬프트", "after": "긍정형(애프터) 영어 프롬프트"
  }
}
...
- ★before/after는 모델(여성)이 등장하는 부정형/긍정형 페어 — 이 제품 카테고리의 진짜 비포/애프터 상태를 구체적으로. 예: 칫솔=치아가 아프고 시린 찡그린 표정 → 하얗고 깨끗한 치아로 환하게 웃는 미소, 드라이기=젖은 머리가 푸석하고 엉킨 지친 모습 → 음이온 케어로 부드럽게 찰랑이는 윤기 머릿결, 고데기=푹 퍼지고 뻗친 머리에 한숨 → 고급진 살롱 스타일링 완성 컷. before는 dull한 조명+답답한 표정(제품 미등장), after는 밝은 하이키+만족스러운 표정(제품 함께)으로 대비를 극대화`;
